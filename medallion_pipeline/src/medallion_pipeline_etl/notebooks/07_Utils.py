"""
Shared utilities for the experimentation in credit card fraud detection project.
"""
...
CATALOG = "workspace"
DATABASE = "abandono_carrito_comercio_electronico"
TRAINING_TABLE = f"{CATALOG}.{DATABASE}.gold_abandonment_training_dataset"
...
TRANSACTION_ID_COLUMN = "session_id"
...
# Reducimos las ventanas para que no agoten todo el dataset disponible
TRAINING_WINDOW_MONTHS = 12   # 12 meses para entrenar
VALIDATION_WINDOW_MONTHS = 4  # 4 meses para validar
...
uc_model_name = f"{catalog}.{database}.abandonment_lr_pipeline"

# The experiment is always co-located with the assets of this project
mlflow_experiment_name = "cart_abandonment_training"
# mlflow_experiment_path = str(
#    Path("/") / "Workspace" / "Users" / current_user / ".experiments" / database / mlflow_experiment_name
#)
# Eliminamos las subcarpetas intermedias que causan el error de "Parent directory does not exist"
mlflow_experiment_path = str(
    Path("/") / "Workspace" / "Users" / current_user / mlflow_experiment_name
)
...
# Localiza este bloque en 07_Utils.py y modifícalo temporalmente:
if delta_semantic_version == 0:
    train_end = datetime(2024, 12, 31) # Ajustado a tus datos
    validation_end = datetime(2025, 4, 30) # Deja mayo y junio para TEST
else:
    # Fuerza el ancla a abril para dejar espacio al Test
    validation_end = datetime(2025, 4, 30) 
...
def apply_class_weights(df):
    ...
    print(f"Abandono: {n_fraud:,} ({pct_fraud:.2f}%), weight = {weight_fraud:.2f}")
    print(f"No Abandono: {n_legit:,} ({pct_legit:.2f}%), weight = {weight_legit:.2f}")
    print()
...
# Lista de columnas excluidas del vector de características (Features)
exclude_columns = [
    LABEL_COLUMN,           # La etiqueta "will_abandon"
    "user_id",              # Identificador del cliente
    "session_id",           # Identificador de la sesión
    "registration_date",    # Dato operativo del perfil
    "event_timestamp",      # Marca de tiempo para el cruce PiT
    "ingestion_timestamp",  # Metadato técnico de la carga
    "source_file",          # Metadato del archivo origen
    "label_available_date", # Evita Data Leakage (información del futuro)
    "__END_AT"              # Columna técnica del Join SCD2 (visto en el Cuaderno 05)
]
...
feature_engineering_statement = "SELECT *, CAST(0 AS INT) AS placeholder_feature FROM __THIS__"
engineered_columns = ["placeholder_feature"]
...
def fig_confusion_matrix(y_true, y_pred, title):
    ...
    for ax, data, fmt, subtitle in [
        ...
        ax.set_xticks([0, 1])
        ax.set_yticks([0, 1])
        ax.set_xticklabels(["No Abandon", "Abandon"])
        ax.set_yticklabels(["No Abandon", "Abandon"])
...
def fig_lr_coefficients(coef_array, feature_names, title):
    ...
    ax.legend(
        handles = [
            Patch(color = color_positive_coef, label = "Indicativo de abandono"),
            Patch(color = color_negative_coef, label = "Indicativo de no abandono o éxito de la compra")
        ],
        loc = "lower right",
        fontsize = 8
    )
...
prob_fraud_column = "prob_abandonment"
